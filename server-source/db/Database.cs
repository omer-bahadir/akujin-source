        ...
        public Database(string host, string database, string user, string password)
        {
            ...
            _con = new MySqlConnection(
                String.Format("Server={0};Database={1};uid={2};password={3};convert zero datetime=True;",
                    host, database ?? "rotmgprod", user ?? "root", password ?? "mHAS9F*#JAS#("));
            _con.Open();
...
AND characters.charId=death.chrId;";
        ...
        public Account Register(string uuid, string password, bool isGuest, XmlData data)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO accounts(uuid, password, name, tag, rank, namechosen, verified, guild, guildRank, guildFame, vaultCount, maxCharSlot, regTime, guest, banned, locked, ignored, gifts, isAgeVerified, authToken) VALUES(@uuid, SHA1(@password), @randomName, @tag, @rank, 0, 0, 0, 0, 0, 1, 2, @regTime, @guest, 0, @empty, @empty, @empty, 1, @authToken);";
            cmd.Parameters.AddWithValue("@uuid", uuid);
